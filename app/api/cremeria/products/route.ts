...
export async function POST(req: Request) {
    try {
        ...
        try {
            ...
            logError('Prisma create error (cremeria):', error)
        ...
        logError('Create cremeria product error (outer):', error)
...
export async function GET(req: Request) {
    try {
        if (!process.env.DATABASE_URL) {
            console.error('GET /api/cremeria/products - missing DATABASE_URL')
        ...
        const CREMERIA_NEGOCIO_ID = Number(process.env.NEXT_PUBLIC_CREMERIA_NEGOCIO_ID || process.env.CREMERIA_NEGOCIO_ID || '3')
        const negocioId = negocioIdParam != null ? Number(negocioIdParam) : CREMERIA_NEGOCIO_ID
        ...
        const msg = errorMessage(error)
        logError('Get cremeria products error:', error)
        ...
        return jsonError('Failed to fetch cremeria products', 500, msg)
    }
}

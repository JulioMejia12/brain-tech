...
import type { RouteContext } from '@/app/api/_utils/route'
import { getNumericRouteParam } from '@/app/api/_utils/route'
...
export async function GET(req: NextRequest, ctx: RouteContext) {
    try {
        ...
        logError('Get cremeria product by id error:', error)
        return jsonError('Failed to fetch cremeria product', 500, errorMessage(error))
    }
}
...
export async function DELETE(req: NextRequest, ctx: RouteContext) {
    try {
        ...
        logError('Delete cremeria product error:', error)
        return jsonError('Failed to delete cremeria product', 500, errorMessage(error))
    }
}
...
export async function PUT(req: NextRequest, ctx: RouteContext) {
    try {
            ...
            if (file instanceof File && typeof file.arrayBuffer === 'function') {
                ...
                try {
                    ...
                    uploadedImageUrl = await new Promise<string>((resolve, reject) => {
                        const uploadStream = cloudinary.v2.uploader.upload_stream({ folder: 'cremeria' }, (error, result) => {
        ...
        try {
            console.info('[PUT] /api/cremeria/products/{id} - updating product', { id: result.value, body, data })
        } catch (e) {
        ...
        logError('Update cremeria product error:', error)
        return jsonError('Failed to update cremeria product', 500, errorMessage(error))
    }
}

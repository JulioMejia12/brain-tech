...
const CREMERIA_NEGOCIO_ID = process.env.NEXT_PUBLIC_CREMERIA_NEGOCIO_ID || '3'

export default function NewCremeriaProductPage() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            fd.append('negocioId', CREMERIA_NEGOCIO_ID)
            fd.append('category', category.trim())
            fd.append('details', JSON.stringify(filteredDetails))
            fd.append('imageFile', imageFile)

            const res = await fetch('/api/cremeria/products', {
                method: 'POST',
                body: fd,
            })
        ...
        <div style={{ background: bg }} className="min-h-screen">
            <main className="max-w-4xl mx-auto p-6">
                <div className="bg-white border border-gray-200 rounded-2xl shadow-lg p-8">
                    <h1 className="text-2xl font-semibold mb-6" style={{ color: secondary }}>Nuevo producto — Cremería</h1>
                    ...
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">Precio de promoción (opcional)</label>
                            <input
                                type="number"
                                step="0.01"
                                className="w-full p-3 rounded-lg border border-gray-200 bg-white text-sm text-gray-900"
